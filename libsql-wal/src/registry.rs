...
impl<FS: FileSystem> WalRegistry<FS> {
    ...
    pub fn open(self: Arc<Self>, db_path: &Path) -> Result<Arc<SharedWal<FS>>> {
        ...
        let (db_size, next_frame_no) = segments
                    .with_head(|log| {
                    ...
                    .unwrap_or((
                        header.db_size.get(),
                        NonZeroU64::new(header.replication_index.get() + 1).unwrap_or(NonZeroU64::new(1).unwrap()),
                    ));

        let current_path = path.join(format!("{namespace}:{next_frame_no:020}.log"));
    ...
    // On shutdown, we checkpoint all the WALs. This require sealing the current log, and when
    ...
    pub fn shutdown(&self) -> Result<()> {
        ...
        for (_, shared) in opened.drain() {
            ...
            // The current log will not be used anymore. It's empty, but we still seal it so that
            // the next startup doesn't find an unsealed log.
